#!/usr/bin/env python3
"""
Spam Detection RAG System using LangChain and Groq DeepSeek.
This system analyzes user messages to identify potential spammers using RAG technology.
"""
...
class SpamDetectorRAG:
    """RAG-based spam detection system."""
    
    def __init__(self, groq_api_key: str = None):
        """Initialize the spam detection RAG system."""
        load_dotenv()
...
def create_sample_user_messages() -> Dict[str, List[UserMessage]]:
    ...
    # Sample data with different user patterns
    users_data = {
        "clear_spammer": [
            UserMessage("clear_spammer", "msg1", "🎉 AMAZING DEAL! Get 90% OFF now! Limited time only!!! Click www.spam-site.com", base_time),
            UserMessage("clear_spammer", "msg2", "🔥 BEST PRICES! Click here for incredible savings! Don't miss out!!!", base_time + timedelta(minutes=2)),
            UserMessage("clear_spammer", "msg3", "💰 MONEY BACK GUARANTEE! Order now and save BIG! Special offer!!!", base_time + timedelta(minutes=3)),
            UserMessage("clear_spammer", "msg4", "⚡ URGENT! Last chance to get this deal! Act NOW!!!", base_time + timedelta(minutes=5)),
            UserMessage("clear_spammer", "msg5", "FREE GIFT CARDS! Download now! www.scam-site.com", base_time + timedelta(minutes=6)),
        ],
        "normal_user": [
            UserMessage("normal_user", "msg1", "Hi there! How are you doing today?", base_time),
            UserMessage("normal_user", "msg2", "I was wondering if you could help me with a question about the project we discussed yesterday.", base_time + timedelta(hours=1)),
            UserMessage("normal_user", "msg3", "Thanks for the information about the deadline. That makes sense now.", base_time + timedelta(hours=2)),
            UserMessage("normal_user", "msg4", "Just wanted to follow up on our conversation. Let me know when you're available to chat.", base_time + timedelta(days=1)),
        ],
        "casual_user": [
            UserMessage("casual_user", "msg1", "Hey! What's up?", base_time),
            UserMessage("casual_user", "msg2", "Did you see the game last night? Amazing!", base_time + timedelta(hours=3)),
            UserMessage("casual_user", "msg3", "Btw, thanks for recommending that restaurant. The food was great!", base_time + timedelta(days=1)),
        ],
        "suspicious_user": [
            UserMessage("suspicious_user", "msg1", "Hey! Check out this link: www.suspicious-site.com", base_time),
            UserMessage("suspicious_user", "msg2", "Free money! Click here to claim your reward!", base_time + timedelta(minutes=10)),
            UserMessage("suspicious_user", "msg3", "Download this file for amazing content!", base_time + timedelta(minutes=15)),
            UserMessage("suspicious_user", "msg4", "Join now and earn $1000 daily!", base_time + timedelta(minutes=20)),
            UserMessage("suspicious_user", "msg5", "Limited spots available! Register today!", base_time + timedelta(minutes=25)),
        ]
    }
...
def main():
    ...
    print("🛡️  Spam Detection RAG System")
    print("=" * 50)
    
    try:
        # Initialize spam detector
        detector = SpamDetectorRAG()
