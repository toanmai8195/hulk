...
class VertxLoadTestClient(
    private val serverHost: String = "localhost",
    private val serverPort: Int = 50052,
    private val metricsPort: Int = 9094
) {
    ...
    init {
        requestCounter = Counter.builder("grpc_vertx_client_requests_total")
            .description("Total gRPC requests sent")
            .register(registry)

        successCounter = Counter.builder("grpc_vertx_client_success_total")
            .register(registry)

        errorCounter = Counter.builder("grpc_vertx_client_errors_total")
            .register(registry)

        droppedCounter = Counter.builder("grpc_vertx_client_dropped_total")
            .description("Requests dropped due to semaphore limit")
            .register(registry)

        requestTimer = Timer.builder("grpc_vertx_client_request_duration")
        ...
        currentRpsGauge = Gauge.builder("grpc_vertx_client_current_rps") {
            calculateCurrentRps()
        }.register(registry)

        targetRpsGauge = Gauge.builder("grpc_vertx_client_target_rps") {
            currentTargetRps.toDouble()
        }.register(registry)

        inflightGauge = Gauge.builder("grpc_vertx_client_inflight") {
    ...
    fun start() {
        ...
        println("🚀 Vert.x Load Test Client starting")
