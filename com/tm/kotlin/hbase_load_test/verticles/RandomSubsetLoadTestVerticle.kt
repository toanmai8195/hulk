...
/**
 * Verticle 5: Load test at 5000 RPS
 * - Filter 1 user from 1000 users
 * - 300 random segments from segment_for_load_test_0001 -> segment_for_load_test_5000
 *   (only within the range of segments that actually exist in HBase)
 */
class RandomSubsetLoadTestVerticle @Inject constructor(
    hbaseClient: IHBaseClient,
    registry: PrometheusMeterRegistry
) : BaseLoadTestVerticle(hbaseClient, registry, "random_subset") {

    companion object {
        const val SEGMENT_MIN = 1
        const val SEGMENT_MAX = 5000
    }
